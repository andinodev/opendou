class_name TestBinauralSpike
extends RefCounted

## Spike 7A: puede una voz salir por Steam Audio dentro de Godot 4.7, y hace algo el HRTF.
##
...
## Captura CAPTURE_FRAMES frames del bus y devuelve los dos canales por separado.
static func _capture(tree: SceneTree, probe) -> Dictionary:
	# Dejar que el anillo de la extension y el filtro se asienten con la direccion nueva.
	for i in range(6):
		await tree.process_frame
		probe.drain()
