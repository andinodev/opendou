...
| B6 | 13 | `IPLReflectionEffectIR` es opaca: la IR **no** se puede exportar a GDScript; el RT60 sale de `reverbTimes` (HYBRID) | Ninguna acción; `OpenDouIRRT60Analyzer` se queda para IRs de usuario — **Resuelta: los RT60 salen de `reverbTimes` (HYBRID)** |
...
|---|---|---|---|
| C1 | **Observación 49**: dentro de una sala con reverb, Godot manda la voz 3D solo al bus de reverb; `target_bus` y la mezcla por buses no la gobiernan | Fase 11 | Fase propia tras la 13: envío de reverb sin el `Area3D` (segundo reproductor por voz o captura del bus de la voz hacia el efecto de convolución). Hasta entonces, el efecto de convolución devuelve seco + húmedo — **Pagada (Fase 15):** envío propio en `steam_audio` (`OpenDouSendBus`/`OpenDouSendStream`); la obs 49 queda acotada a `godot` |
| C2 | Los cuatro nodos de la Fase 10 no tienen demo (guarda de cobertura con `EXPECTED_UNCOVERED`) | Fase 11 | Una demo «El estanque» (oyente bajo el agua, volumen de entorno, indicador, guardia que oye) en la Fase 15 o antes — **→ Fase 16:** la escena grande instancia los cuatro nodos y recorta `EXPECTED_UNCOVERED` |
| C3 | `OpenDouSplineEmitter3D` y `OpenDouMultiPositionEmitter3D` fuera del sistema de voces (obs 47) | Fases 9 y 11 | Integrarlos como fuentes de posición del pool (como los emisores de nodo en steam_audio) en una tarea aparte — **Pagada (Fase 15):** proveedores de posición (`position_provider`); la voz es del pool |
| C4 | Aceleración nativa del medidor LUFS (91 ms por segundo de audio en GDScript) | Fase 8 | Un `AudioEffect` nativo cuando B5 esté confirmado: mismo mecanismo que la convolución — **Pagada (Fase 15):** `OpenDouLoudnessTap`, 0.8 ms/s frente a 73 |
| C5 | Latencia del altavoz de mundo (`BUS_CAPTURE`) no medida | Fase 11 | Medirla con un impulso en el bus origen y el bus del emisor — **Pagada (Fase 15):** 107 ms, estable; afirmado en `test_world_speaker_latency` |
