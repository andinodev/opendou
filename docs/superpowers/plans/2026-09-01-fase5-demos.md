...
`addons/opendou/data/music_suites.json` está **vacío** (`{}`): hoy el reproductor de
música siempre cae a sus stems sintéticos de reserva y el cargador de suites es código
muerto. Esta tarea le da su primera entrada real, con **un bus por stem**, que es lo
que permite medir el desplazamiento de energía por stem en lugar de mirar
`volume_db`.
...
3. `addons/opendou/data/music_suites.json` estaba **vacío**: el cargador de suites era
   código muerto y el reproductor siempre caía a sus stems de reserva. La Tarea 7 le da
   su primera entrada real.
