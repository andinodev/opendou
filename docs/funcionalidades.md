...
| Nodo | Hereda de | Qué hace | Estado |
|---|---|---|---|
| `OpenDouRoom3D` | `Area3D` | Sala acústica: volumen, RT60 efectivo, absorción por material; se registra en el grafo de salas y pide un bus de reverb a `OpenDouReverbBusPool`, que agrupa salas por perfil y les asigna hasta ocho `AudioEffectReverb` nativos escalonados por RT60. Se desregistra al salir del árbol. Desde la Fase 13, `reverb_mode = CONVOLUTION`: la IR trazada por Steam Audio contra el bake, centrada en el oyente, como efecto nativo en el bus de la sala; sin extensión cae a Sabine con el RT60 real si lo hubo Fase 15: en `steam_audio` con extensión la sala usa el **envío propio** (`send_id` del bus del pool) y no el `reverb_bus` del `Area3D`: la voz vuelve a su `target_bus`; en `godot` sigue el `Area3D` (observación 49 acotada a ese backend). | ✅ |
...
| **Envío propio de reverb** (Fase 15) | `OpenDouSendBus` acumula en mono, por bus del pool, la señal de cada voz antes del HRTF (ya atenuada, filtrada y con efecto directo) con la cantidad de su sala; `OpenDouSendStream`, reproducido por un `AudioStreamPlayer` en el bus de reverb, la vuelca: el bus tiene un reproductor real y Godot lo mantiene activo. Las salas dejan de usar `reverb_bus_enabled` y la voz vuelve a su `target_bus` (ducking, instantáneas y mezcla por buses la alcanzan de nuevo) | Caja de hormigón: la voz suena en su bus destino (−7.4 dB; antes −200), el bus de reverb tiene cola (−13.8 dB) y con envío 0 calla (−165); con `wet = 0` no lleva seco (−180). Control `godot`: destino −180, el `Area3D` sigue mandando | ✅ |
...
| **Medidor LUFS** | `OpenDouLoudnessMeter` | BS.1770-4: filtro K, momentánea, a corto plazo, integrada con compuerta, pico muestral. −23.26 LUFS para el tono de calibración; presupuesto por demo en `tests/loudness_budget.txt`. Apagado por defecto (91 ms por segundo de audio en GDScript); lectura en el HUD de depuración del juego Fase 15: con extensión, `OpenDouLoudnessTap` hace el filtro K y los bloques en el hilo de audio (0.8 ms por segundo de audio frente a 73 en GDScript; misma integrada −23.26). | ✅ |
