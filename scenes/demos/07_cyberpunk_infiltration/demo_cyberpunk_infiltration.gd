...
func _physics_process(delta: float) -> void:
	...
	# 3. Dynamic Pillar Raycast Occlusion in Sector 4
	var listener_pos = player.global_position if player else Vector3.ZERO
	_update_pillar_occlusion(listener_pos)
	
	# 4. Update room acoustics based on player position
	_update_room_acoustics(listener_pos)
	
	# 5. Refresh HUD telemetry
	_update_hud()

func _update_pillar_occlusion(listener_pos: Vector3) -> void:
	if not player or player.global_position.x < 35.0:
...
func _update_hud() -> void:
	...
	if lbl_voices and voice_pool:
		...
		lbl_voices.text = "Voice Pool: %d Physical / 0 Virtual (Cap: 16)" % phys
